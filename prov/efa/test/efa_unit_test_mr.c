...
void test_efa_mr_validate_regattr_uninitialized_iface(struct efa_resource **state)
{
	...
	original_initialized = hmem_ops[FI_HMEM_CUDA].initialized;
	...
	hmem_ops[FI_HMEM_CUDA].initialized = original_initialized;

	free(buf);
}

/**
 * @brief Test core efa_mr structure and inline functions
 */
void test_efa_mr_is_hmem_functions(struct efa_resource **state)
{
	struct efa_mr efa_mr = { 0 };

	/* Test FI_HMEM_SYSTEM */
	efa_mr.iface = FI_HMEM_SYSTEM;
	assert_false(efa_mr_is_hmem(&efa_mr));
	assert_false(efa_mr_is_cuda(&efa_mr));
	assert_false(efa_mr_is_neuron(&efa_mr));
	assert_false(efa_mr_is_synapseai(&efa_mr));
	assert_false(efa_mr_is_rocr(&efa_mr));

	/* Test FI_HMEM_CUDA */
	efa_mr.iface = FI_HMEM_CUDA;
	assert_true(efa_mr_is_hmem(&efa_mr));
	assert_true(efa_mr_is_cuda(&efa_mr));
	assert_false(efa_mr_is_neuron(&efa_mr));
	assert_false(efa_mr_is_synapseai(&efa_mr));
	assert_false(efa_mr_is_rocr(&efa_mr));
	...
	assert_true(efa_mr_is_hmem(&efa_mr));
	assert_false(efa_mr_is_cuda(&efa_mr));
	assert_true(efa_mr_is_neuron(&efa_mr));
	assert_false(efa_mr_is_synapseai(&efa_mr));
	assert_false(efa_mr_is_rocr(&efa_mr));
	...
	assert_false(efa_mr_is_neuron(&efa_mr));
	assert_false(efa_mr_is_synapseai(&efa_mr));
	assert_true(efa_mr_is_rocr(&efa_mr));

	/* Test FI_HMEM_SYNAPSEAI */
	efa_mr.iface = FI_HMEM_SYNAPSEAI;
	assert_true(efa_mr_is_hmem(&efa_mr));
	assert_false(efa_mr_is_cuda(&efa_mr));
	assert_false(efa_mr_is_neuron(&efa_mr));
	assert_true(efa_mr_is_synapseai(&efa_mr));
	assert_false(efa_mr_is_rocr(&efa_mr));

	/* Test NULL pointer */
	assert_false(efa_mr_is_hmem(NULL));
	assert_false(efa_mr_is_cuda(NULL));
	assert_false(efa_mr_is_neuron(NULL));
	assert_false(efa_mr_is_synapseai(NULL));
	assert_false(efa_mr_is_rocr(NULL));
}
 ...
 *
 * Tests case 1: New registration is smaller and fully encapsulated within a previous region.
 ...
 *
 * Tests case 3: New registration and previous registrations have no full encapsulations.

 ...
 *
 * Sets up ibv_wr and efadv mocks, calls efa_ibv_post_write with
 * FI_EFA_WR_HIGH_PPS, and lets the caller control whether the hint
 * should fire via @p enable_high_pps.
 */
static void test_efa_ibv_post_write_high_pps_impl(struct efa_resource *resource,
						  bool enable_high_pps)
{
	struct efa_unit_test_buff local_buff;
	struct ibv_sge sge;
	...
	efa_env.enable_high_pps = enable_high_pps;
	...
	efa_ibv_post_write(qp, &sge, 1, 123456, 0x87654321, 0, 0,
			   FI_EFA_WR_HIGH_PPS, &fake_ah, 0, 0);
 ...
 * when efa_env.enable_high_pps is true and FI_EFA_WR_HIGH_PPS flag is set.
 */
void test_efa_ibv_post_write_high_pps_calls_processing_hint(struct efa_resource **state)
{
#if HAVE_EFADV_WR_PROCESSING_HINTS
	test_efa_ibv_post_write_high_pps_impl(*state, true);
#else
	skip();
#endif
}
 ...
 * when efa_env.enable_high_pps is false.
 */
void test_efa_ibv_post_write_high_pps_not_called_when_disabled(struct efa_resource **state)
{
#if HAVE_EFADV_WR_PROCESSING_HINTS
	test_efa_ibv_post_write_high_pps_impl(*state, false);
#else
	skip();
#endif
}

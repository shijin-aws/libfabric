					...
					uint64_t flags)
{
	...
	qp->ops->wr_rdma_read(qp, msg->rma_iov[0].key, msg->rma_iov[0].addr);
	...
	qp->ops->wr_set_sge_list(qp, msg->iov_count, sge_list);
					 ...
					 uint64_t flags)
{
	...
	if (flags & FI_REMOTE_CQ_DATA) {
		qp->ops->wr_rdma_write_imm(qp, msg->rma_iov[0].key,
				      msg->rma_iov[0].addr, msg->data);
	} else {
		qp->ops->wr_rdma_write(qp, msg->rma_iov[0].key, msg->rma_iov[0].addr);
	}

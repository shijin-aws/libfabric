					...
					uint64_t flags)
{
	...
	efaibv_wr_rdma_read(qp, qp->ibv_qp_ex, msg->rma_iov[0].key, msg->rma_iov[0].addr);
	...
	efaibv_wr_set_sge_list(qp, qp->ibv_qp_ex, msg->iov_count, sge_list);
					 ...
					 uint64_t flags)
{
	...
	if (flags & FI_REMOTE_CQ_DATA) {
		efaibv_wr_rdma_write_imm(qp, qp->ibv_qp_ex, msg->rma_iov[0].key,
				      msg->rma_iov[0].addr, msg->data);
	} else {
		efaibv_wr_rdma_write(qp, qp->ibv_qp_ex, msg->rma_iov[0].key, msg->rma_iov[0].addr);
	}

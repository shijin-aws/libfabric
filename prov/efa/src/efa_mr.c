...
static int efa_mr_hmem_setup(struct efa_mr *efa_mr,
							 ...
							 uint64_t flags)
{
	/* Note: flags are now handled in RDM layer for efa_rdm_mr */
	...
	if (efa_mr->iface == FI_HMEM_CUDA) {
		...
		efa_mr->device = attr->device.rocr;
	} else if (attr->iface == FI_HMEM_NEURON) {
		efa_mr->device = attr->device.neuron;
	} else if (attr->iface == FI_HMEM_SYNAPSEAI) {
		efa_mr->device = attr->device.synapseai;
	}

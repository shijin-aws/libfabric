...
static int efa_mr_cache_close(fid_t fid)
{
	...
	ofi_mr_cache_delete(efa_rdm_mr->efa_mr.domain->cache, efa_rdm_mr->entry);
...
static int efa_mr_dereg_impl(struct efa_mr *efa_mr)
{
	...
	if (efa_rdm_mr->peer.iface == FI_HMEM_CUDA &&
		...
		err = ofi_hmem_dev_unregister(FI_HMEM_CUDA, (uint64_t)efa_rdm_mr->peer.hmem_data);
		if (err) {
			EFA_WARN(FI_LOG_MR,
...
{
	struct efa_rdm_mr *efa_rdm_mr = (struct efa_rdm_mr *)efa_mr;
	struct efa_hmem_info *p_info = &g_efa_hmem_info[efa_rdm_mr->peer.iface];
	...
	if (flags & FI_MR_DMABUF) {
		...
		if (!dmabuf_mr)  {
			efa_mark_dmabuf_fail(p_info, efa_rdm_mr->peer.iface);
		} else {
	...
	if (DMABUF_IS_SUPPORTED(p_info)) {
		...
		if (ret == FI_SUCCESS) {
			 ...
			 * after registration
			 */
			(void) ofi_hmem_put_dmabuf_fd(efa_rdm_mr->peer.iface, dmabuf_fd);
			...
			EFA_INFO(FI_LOG_MR,
				 ...
				 fi_tostr(&efa_rdm_mr->peer.iface, FI_TYPE_HMEM_IFACE));
			efa_mark_dmabuf_fail(p_info, efa_rdm_mr->peer.iface);
			return ibv_reg_mr(efa_mr->domain->ibv_pd,
					  (void *)mr_attr->mr_iov->iov_base,
					  mr_attr->mr_iov->iov_len, access);
		...
		EFA_WARN(FI_LOG_MR,
			 ...
			 efa_rdm_mr->peer.iface, ret, fi_strerror(-ret));
				...
				uint64_t flags)
{
	...
	struct efa_rdm_mr *existing_rdm_mr = (struct efa_rdm_mr *)existing_mr;

	if (existing_rdm_mr->peer.iface != FI_HMEM_CUDA) {
		...
		EFA_WARN(FI_LOG_DOMAIN, "key %ld already assigned to buffer: %p hmem_iface: %s length: %ld\n",
			 ...
			 fi_tostr(&existing_rdm_mr->peer.iface, FI_TYPE_HMEM_IFACE),
...
static int efa_mr_reg_impl(struct efa_mr *efa_mr, uint64_t flags, const struct fi_mr_attr *mr_attr)
{
	...
	ret = efa_mr_hmem_setup(efa_rdm_mr, mr_attr, flags);
	if (ret)
		return ret;
	...
	if ((mr_attr->iface == FI_HMEM_CUDA || mr_attr->iface == FI_HMEM_ROCR)
		...
		if (!efa_mr->ibv_mr) {
			...
			if (efa_rdm_mr->peer.iface == FI_HMEM_CUDA &&
			    (efa_rdm_mr->peer.flags & OFI_HMEM_DATA_DEV_REG_HANDLE)) {
				assert(efa_rdm_mr->peer.hmem_data);
				ofi_hmem_dev_unregister(FI_HMEM_CUDA, (uint64_t)efa_rdm_mr->peer.hmem_data);
			}

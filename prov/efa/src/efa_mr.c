...
{
	struct efa_hmem_info *p_info = &g_efa_hmem_info[mr_attr->iface];
	...
	if (flags & FI_MR_DMABUF) {
		...
		if (!dmabuf_mr)  {
			efa_mark_dmabuf_fail(p_info, mr_attr->iface);
		} else {
		      ...
		      struct fid_mr **mr, void *context)
{
	...
	*mr = NULL;
	...
	efa_mr->mr_fid.fid.ops = &efa_rdm_mr_ops;

...
static void efa_hw_cntr_no_progress(struct util_cntr *cntr)
{
	/*
	 * Hardware counters do not internally poll the completion queue.
	 * Unlike software counters, hardware counters are incremented directly
	 * by the device and do not require CQ entries to be consumed to update
	 * their value. However, the CQ still accumulates completion entries for
	 * each operation.
	 *
	 * EFA-direct claims FI_RM_ENABLED and returns -FI_EAGAIN on data transfer
		     ...
		     struct efadv_comp_cntr_init_attr *efa_cc_attr)
{
	...
	ret = efa_cntr_construct(cntr, domain, attr, efa_hw_cntr_no_progress, context);
	if (ret) {

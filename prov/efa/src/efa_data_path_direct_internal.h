 ...
 *
 * All functions in this file are marked as MAYBE_INLINE for optimal
 ...
 */
MAYBE_INLINE void efa_data_path_direct_rq_ring_doorbell(struct efa_data_path_direct_rq *rq,
						uint16_t pc)
{
 ...
 * @param pc Producer counter value to write to doorbell
 */
MAYBE_INLINE void efa_sq_ring_doorbell(struct efa_data_path_direct_sq *sq, uint16_t pc)
{
 ...
 *
 * @note This function assumes the queue has available space (validated elsewhere)
 */
MAYBE_INLINE uint32_t efa_wq_get_next_wrid_idx(struct efa_data_path_direct_wq *wq,
					       uint64_t wr_id)
{
	uint32_t wrid_idx;
	...
	/* Advance to next pool entry - will never overflow as validate succeeded */
...
{
	/* Note: enum efa_errno status codes are precisely enum efa_io_comp_status */
	switch (status) {
...
efa_sub_cq_get_cqe(struct efa_data_path_direct_cq *cqd, int entry)
{
 ...
 *
 * @note This function mirrors cq_next_sub_cqe_get from rdma-core
 */
MAYBE_INLINE struct efa_io_cdesc_common *
efa_data_path_direct_next_sub_cqe_get(struct efa_data_path_direct_cq *data_path_direct)
{
	/* Mirror of cq_next_sub_cqe_get from rdma-core */
	struct efa_io_cdesc_common *cqe;
	uint32_t current_index;

	current_index = efa_data_path_direct_get_current_index(data_path_direct);
	cqe = efa_sub_cq_get_cqe(data_path_direct, current_index);
 ...
 */
MAYBE_INLINE void efa_data_path_direct_process_ex_cqe(struct efa_ibv_cq *ibv_cq,
					      struct efa_qp *qp)
{
	...
	if (EFA_GET(&cqe->flags, EFA_IO_CDESC_COMMON_Q_TYPE) ==
	    EFA_IO_SEND_QUEUE) {
		ibv_cq->data_path_direct.cur_wq = &qp->data_path_direct_qp.sq.wq;
 ...
 */
MAYBE_INLINE void efa_wq_put_wrid_idx(struct efa_data_path_direct_wq *wq,
				      uint32_t wrid_idx)
{
 ...
 */
MAYBE_INLINE int efa_data_path_direct_wq_initialize(struct efa_data_path_direct_wq *wq,
					    uint32_t wqe_cnt,
 ...
 */
MAYBE_INLINE void efa_data_path_direct_wq_finalize(struct efa_data_path_direct_wq *wq)
{
	if (wq->wrid) {
		free(wq->wrid);
		wq->wrid = NULL;
				 ...
				 uint64_t addr, uint32_t lkey, uint32_t length)
{
	...
	tx_buf->buf_addr_lo = addr & 0xffffffff; /* Lower 32 bits */
	tx_buf->buf_addr_hi = addr >> 32;        /* Upper 32 bits */
}
 ...
 * @param wr_flags Work request flags (currently unused)
					...
					struct ibv_recv_wr *wr)
{
	...
	if (OFI_UNLIKELY(qp->data_path_direct_qp.rq.wq.wqe_posted -
		...
		EFA_WARN(FI_LOG_EP_DATA,
			 ...
			 "wqe_cnt[%u]\n",
			 qp->ibv_qp->qp_num, qp->data_path_direct_qp.rq.wq.wqe_posted,
			 qp->data_path_direct_qp.rq.wq.wqe_completed,
			 qp->data_path_direct_qp.rq.wq.wqe_cnt);
		return ENOMEM;
	}
 ...
 * @param sq Pointer to the send queue structure
 */
MAYBE_INLINE void efa_sq_advance_post_idx(struct efa_data_path_direct_sq *sq)
{
...
MAYBE_INLINE void
efa_data_path_direct_send_wr_set_imm_data(struct efa_io_tx_wqe *tx_wqe, __be32 imm_data)
{
...
MAYBE_INLINE void efa_data_path_direct_send_wr_post_working(struct efa_data_path_direct_sq *sq,
						    bool force_doorbell)
{
...
efa_data_path_direct_send_wr_common(struct efa_qp *qp, enum efa_io_send_op_type op_type)
{

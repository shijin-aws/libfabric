...
/*
 * @brief wait for send to finish and flush the ibv cq
 *
 ...
 */
static inline
void efa_rdm_ep_wait_send_and_flush_ibv_cq(struct efa_rdm_ep *efa_rdm_ep)
{
	...
	while (efa_rdm_ep_has_unfinished_send(efa_rdm_ep)) {
		/* poll cq until empty */
		if (efa_rdm_ep->tx_cq)
			efa_rdm_cq_poll_ibv_cq(-1, &efa_rdm_ep->tx_cq->ibv_cq);
		if (efa_rdm_ep->rx_cq)
			efa_rdm_cq_poll_ibv_cq(-1, &efa_rdm_ep->rx_cq->ibv_cq);
...
static int efa_rdm_ep_close(struct fid *fid)
{
	...
	if (efa_rdm_ep->base_ep.efa_qp_enabled)
		efa_rdm_ep_wait_send_and_flush_ibv_cq(efa_rdm_ep);

...
static int efa_rdm_ep_close(struct fid *fid)
{
	...
	if (efa_rdm_ep->base_ep.user_recv_qp)
		EFA_WARN(FI_LOG_EP_CTRL, "closing ep %p on host id %lx with user recv ibv_qp %p, qpn: %d, efa_domain %p, name: %s, cxt: %p\n", efa_rdm_ep, efa_rdm_ep->host_id, efa_rdm_ep->base_ep.user_recv_qp->ibv_qp, efa_rdm_ep->base_ep.user_recv_qp->qp_num, efa_rdm_ep_domain(efa_rdm_ep), efa_rdm_ep_domain(efa_rdm_ep)->util_domain.name, efa_rdm_ep_domain(efa_rdm_ep)->device->ibv_ctx);
...
static int efa_rdm_ep_ctrl(struct fid *fid, int command, void *arg)
{
	...
	case FI_ENABLE:
		...
		if (ep->base_ep.user_recv_qp)
			EFA_WARN(FI_LOG_EP_CTRL, "enabled ep %p on host id %lx with user recv ibv_qp %p, qpn: %d, efa_domain: %p, name: %s, ctx: %p\n", ep, ep->host_id, ep->base_ep.user_recv_qp->ibv_qp, ep->base_ep.user_recv_qp->qp_num, efa_rdm_ep_domain(ep), efa_rdm_ep_domain(ep)->util_domain.name, efa_rdm_ep_domain(ep)->device->ibv_ctx);

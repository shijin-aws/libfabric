...
int efa_rdm_ep_flush_queued_blocking_copy_to_hmem(struct efa_rdm_ep *ep)
{
	...
	for (i = 0; i < ep->queued_copy_num; ++i) {
		...
		if (((struct efa_rdm_mr *)desc)->flags & OFI_HMEM_DATA_DEV_REG_HANDLE) {
			/* Safe cast: In RDM context, efa_mr is always part of efa_rdm_mr */
			assert(((struct efa_rdm_mr *)desc)->hmem_data);
			bytes_copied[i] = ofi_dev_reg_copy_to_hmem_iov(
								desc->iface,
								(uint64_t)((struct efa_rdm_mr *)desc)->hmem_data,
					   ...
					   bool *restrict gdrcopy_available)
{
	...
	*gdrcopy_available = ((struct efa_rdm_mr *)efa_mr)->flags & OFI_HMEM_DATA_DEV_REG_HANDLE;
				     ...
				     struct efa_rdm_ope *rxe)
{
	...
	if (rxe->cuda_copy_method != EFA_RDM_CUDA_COPY_LOCALREAD) {
		...
		if (rxe->bytes_copied + pke->payload_size == rxe->total_len) {
			...
			ofi_dev_reg_copy_to_hmem_iov(FI_HMEM_CUDA, (uint64_t)((struct efa_rdm_mr *)desc)->hmem_data,
			                             rxe->iov, rxe->iov_count,
			                             segment_offset + ep->msg_prefix_size,

			  ...
			  int pkt_entry_cnt, uint64_t flags)
{
	...
	qp = ep->base_ep.qp;
	if (!ep->base_ep.is_send_wr_started) {
		ibv_wr_start(qp->ibv_qp_ex);
		ep->base_ep.is_send_wr_started = true;
	}
	...
	if (!(flags & FI_MORE)) {
		...
		ep->base_ep.is_send_wr_started = false;
	}
...
int efa_rdm_pke_write(struct efa_rdm_pke *pkt_entry)
{
	...
	qp = ep->base_ep.qp;
	if (!ep->base_ep.is_write_wr_started) {
		ibv_wr_start(qp->ibv_qp_ex);
		ep->base_ep.is_write_wr_started = true;
	}
	qp->ibv_qp_ex->wr_id = (uintptr_t)pkt_entry;
	...
	if (!(txe->fi_flags & FI_MORE)) {
		err = ibv_wr_complete(qp->ibv_qp_ex);
		ep->base_ep.is_write_wr_started = false;
	}

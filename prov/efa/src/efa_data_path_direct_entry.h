 ...
 *
 * All functions are marked as ENTRY_FUN (static inline) for optimal
 ...
 */
ENTRY_FUN int efa_data_path_direct_post_recv(struct efa_qp *qp, struct ibv_recv_wr *wr,
 ...
 */
ENTRY_FUN int efa_data_path_direct_wr_complete(struct efa_qp *qp)
{
 ...
 */
ENTRY_FUN void efa_data_path_direct_wr_rdma_read(struct efa_qp *efaqp, uint32_t rkey,
 ...
 */
ENTRY_FUN void efa_data_path_direct_wr_rdma_write(struct efa_qp *efaqp, uint32_t rkey,
			  uint64_t remote_addr)
{
	struct efa_io_tx_wqe *tx_wqe;
 ...
 */
ENTRY_FUN void efa_data_path_direct_wr_rdma_write_imm(struct efa_qp *efaqp,
			      uint32_t rkey,
			      uint64_t remote_addr,
			      __be32 imm_data)
{
 ...
 * @param efaqp Pointer to the EFA queue pair
 */
ENTRY_FUN void efa_data_path_direct_wr_send(struct efa_qp *efaqp)
{
 ...
 */
ENTRY_FUN void efa_data_path_direct_wr_send_imm(struct efa_qp *efaqp, __be32 imm_data)
{
	struct efa_io_tx_wqe *tx_wqe;
 ...
 */
ENTRY_FUN void
efa_data_path_direct_wr_set_inline_data_list(struct efa_qp *efa_qp, size_t num_buf,
 ...
 */
ENTRY_FUN void efa_data_path_direct_wr_set_sge_list(struct efa_qp *efa_qp,
			    size_t num_sge,
 ...
 */
ENTRY_FUN void efa_data_path_direct_wr_set_ud_addr(struct efa_qp *efaqp,
			   struct efa_ah *ah,
			   uint32_t remote_qpn,
 ...
 * @param qp Pointer to the EFA queue pair
 */
ENTRY_FUN void efa_data_path_direct_wr_start(struct efa_qp *qp)
{
 ...
 */
ENTRY_FUN int efa_data_path_direct_start_poll(struct efa_ibv_cq *ibv_cq,
		      struct ibv_poll_cq_attr *attr)
{
	...
	data_path_direct->cur_cqe =
		efa_data_path_direct_next_sub_cqe_get(data_path_direct);
...
ENTRY_FUN uint32_t efa_data_path_direct_wc_read_qp_num(struct efa_ibv_cq *ibv_cq)
{
...
ENTRY_FUN int efa_data_path_direct_next_poll(struct efa_ibv_cq *ibv_cq)
{
...
ENTRY_FUN void efa_data_path_direct_end_poll(struct efa_ibv_cq *ibv_cq)
{
	struct efa_io_cdesc_common *cqe = ibv_cq->data_path_direct.cur_cqe;
...
ENTRY_FUN uint32_t efa_data_path_direct_wc_read_vendor_err(struct efa_ibv_cq *ibv_cq)
{
	return ibv_cq->data_path_direct.cur_cqe->status;
}

ENTRY_FUN uint32_t efa_data_path_direct_wc_read_byte_len(struct efa_ibv_cq *ibv_cq)
{
	struct efa_io_cdesc_common *cqe;
...
ENTRY_FUN unsigned int efa_data_path_direct_wc_read_wc_flags(struct efa_ibv_cq *ibv_cq)
{
...
ENTRY_FUN __be32 efa_data_path_direct_wc_read_imm_data(struct efa_ibv_cq *ibv_cq)
{
...
ENTRY_FUN uint32_t efa_data_path_direct_wc_read_src_qp(struct efa_ibv_cq *ibv_cq)
{
	struct efa_io_rx_cdesc *rcqe;
...
ENTRY_FUN uint32_t efa_data_path_direct_wc_read_slid(struct efa_ibv_cq *ibv_cq)
{
	struct efa_io_rx_cdesc *rcqe;
...
ENTRY_FUN int efa_data_path_direct_wc_read_sgid(struct efa_ibv_cq *ibv_cq,
			union ibv_gid *sgid)
{
...
ENTRY_FUN bool efa_data_path_direct_wc_is_unsolicited(struct efa_ibv_cq *ibv_cq)
{

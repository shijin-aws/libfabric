...
static uint32_t local_comp_mem_id;
static uint32_t remote_comp_mem_id;
 ...
 * Returns the opaque signal_id (and comp_mem_id for later teardown).
 */
static int register_memset_signal(volatile uint32_t *sem,
				   uint32_t *signal_id, uint32_t *comp_mem_id)
{
	...
	ret = sig_ops->create_comp_mem_op(domain, &mem_attr, comp_mem_id);
	if (ret) {
	...
	sig_attr.comp_mem_id = *comp_mem_id;

	ret = sig_ops->register_signal(domain, &sig_attr, signal_id);
	if (ret) {
		FT_PRINTERR("register_signal", ret);
		sig_ops->destroy_comp_mem_op(domain, *comp_mem_id);
		return ret;
	}
...
static int run(void)
{
	...
	ret = ft_finalize();
out:
	if (sig_ops && (local_signal_id || local_comp_mem_id)) {
		sig_ops->deregister_signal(domain, local_signal_id);
		sig_ops->destroy_comp_mem_op(domain, local_comp_mem_id);
	}
	if (sig_ops && (remote_signal_id || remote_comp_mem_id)) {
		/* Only the target owns the remote signal resource. */
		if (!opts.dst_addr) {
			sig_ops->deregister_signal(domain, remote_signal_id);
			sig_ops->destroy_comp_mem_op(domain, remote_comp_mem_id);
		}
	}
	free((void *) local_sem);

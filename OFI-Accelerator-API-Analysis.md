...
| **Backpressure** | Not addressed | **Dual:** (a) un-rung depth ≤ max_batch (doorbell staging limit), (b) ring overflow via `(chunk_next - *hw_cntr) & 0x7FFF ≤ sq_size` | CQ-based: `scnt - ccnt < tx_depth` | Hidden inside `fi_acc_write()` — provider enforces both limits internally |
| **Doorbell control** | Not addressed | **Deferred aggregation:** `ncclGinOptFlagsAggregateRequests` skips doorbell; later group drains all deferred WQEs with one ring | `flush_sq_wrs()` after batch of ≤16 WQEs | Hidden inside `fi_acc_write()` — `FI_MORE` flag defers doorbell; provider rings when max_batch reached or `FI_MORE` absent |

...
: Creates a completion memory operation that writes a value to a target
  ...
  *location* (*struct fi_efa_memory_location*) points at host memory (a virtual
  ...
  Returns an opaque *comp_mem_id*.

*destroy_comp_mem_op*
: Destroys a completion memory operation previously created by
  *create_comp_mem_op*.

*register_signal*
: Creates a signal over a completion memory operation
  (*FI_EFA_COMP_SIGNAL_MEM_OP*, referencing a *comp_mem_id*) or an existing
  libfabric counter (*FI_EFA_COMP_SIGNAL_CNTR_INC*, referencing a *struct
  fid_cntr \**), described by *struct fi_efa_comp_signal_attr*. Returns an
  opaque *signal_id*. For a signal that fires on the receiver (a remote
  signal), the *signal_id* must be communicated out of band from the receiver
  to the sender.

*deregister_signal*
: Destroys a signal previously created by *register_signal*.
...
```c
/* Value width written by a MEMSET completion memory operation. */
...
uint32_t comp_mem_id, signal_id;
uint32_t sem = 0;
...
ops->create_comp_mem_op(domain, &mem, &comp_mem_id);

struct fi_efa_comp_signal_attr sig = {
	.type = FI_EFA_COMP_SIGNAL_MEM_OP, .comp_mem_id = comp_mem_id,
};
ops->register_signal(domain, &sig, &signal_id);
/* send signal_id to the sender out of band */
```

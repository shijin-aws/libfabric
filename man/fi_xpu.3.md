...
The XPU context is also passed to `fi_av_lookup2` and `fi_mr_get_desc` so
...
: Bitmask of XPU capabilities supported by this provider. The application
  ...
  - `FI_XPU_CAP_EP`: Provider supports XPU endpoints. This includes:
    ...
    `fi_mr_get_desc` with FI_XPU to retrieve raw MR descriptors.
...
*mr_desc_size*
: Size in bytes of the raw descriptor returned by `fi_mr_get_desc` (when
...
`fi_av_lookup2` and `fi_mr_get_desc` calls. Different XPU contexts (targeting
...
returning a handle usable by device-side data transfer functions.
...
`fi_cq_export_xpu` exports the CQ for device access, returning a handle
usable by the device-side completion functions (`fi_xpu_cq_read`,
...
`fi_cntr_export_xpu` exports the counter for device access, returning a
handle usable by the device-side counter functions (`fi_xpu_cntr_read`,
...
a buffer, calls `fi_mr_get_desc` with the `FI_XPU` flag and the XPU context
...
`fi_mr_get_desc` is an extended descriptor query that accepts flags and an
...
struct fid_xpu {
    uint32_t fclass;    /* FI_CLASS_XPU_EP, _CQ, _CNTR */
...
The generic dispatch functions cast the exported handle to `struct fid_xpu *`
and switch on `fid->prov_id` to route to the appropriate provider-specific
implementation. Provider-specific device headers define `fi_xpu_<op>_<prov>()`
functions. When a provider implements its device-side support, its header is
included in `fi_xpu_device.h` and the corresponding cases are added to each
dispatch switch.
...
size_t ep_size, cq_size, cntr_size;
fi_ep_export_xpu(ep, 0, &xpu_ep, &ep_size);
fi_cq_export_xpu(cq, 0, &xpu_cq, &cq_size);
fi_cntr_export_xpu(cntr, 0, &xpu_cntr, &cntr_size);
...
len = desc_size;
fi_mr_get_desc(mr, raw_desc, &len, FI_XPU, xpu_ctx);

/* 12. Copy raw AV addr and MR desc to device-accessible memory */
 ...
 *     }
 */
launch_kernel(my_kernel, xpu_ep, xpu_cntr, gpu_addr, gpu_desc, gpu_buf);
